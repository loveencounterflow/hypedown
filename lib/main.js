  ...
  Hypedown_transforms = class Hypedown_transforms {
    ...
    $parse_md_hashes({mode, tid}) {
      ...
      return function(d, send) {
        ...
        send(XXX_new_token('^æ1^', d, 'html', 'tag', name, `<${name}>`));
        return null;
      };
    }
            ...
            if (within.two) {
              if (within.one) {
                if (start_of.one > start_of.two) {
                  exit.one();
                  send(XXX_new_token('^æ3^', d, 'html', 'tag', 'i', '</i>'));
                  exit.two();
                  send(XXX_new_token('^æ4^', d, 'html', 'tag', 'b', '</b>'));
                  enter.one(d.start);
                  send(XXX_new_token('^æ5^', d, 'html', 'tag', 'i', '<i>'));
                } else {
                  exit.two();
                  send(XXX_new_token('^æ6^', d, 'html', 'tag', 'b', '</b>'));
                }
              } else {
                exit.two();
                send(XXX_new_token('^æ7^', d, 'html', 'tag', 'b', '</b>'));
              }
            } else {
              enter.two(d.start);
              send(XXX_new_token('^æ8^', d, 'html', 'tag', 'b', '<b>'));
            }
            break;
          //...................................................................................................
          case star3_tid:
            ...
            if (within.one) {
              if (within.two) {
                if (start_of.one > start_of.two) {
                  exit.one();
                  send(XXX_new_token('^æ9^', d, 'html', 'tag', 'i', '</i>'));
                  exit.two();
                  send(XXX_new_token('^æ10^', d, 'html', 'tag', 'b', '</b>'));
                } else {
                  exit.two();
                  send(XXX_new_token('^æ11^', d, 'html', 'tag', 'b', '</b>'));
                  exit.one();
                  send(XXX_new_token('^æ12^', d, 'html', 'tag', 'i', '</i>'));
                }
              } else {
                exit.one();
                send(XXX_new_token('^æ13^', d, 'html', 'tag', 'i', '</i>'));
                enter.two(d.start);
                send(XXX_new_token('^æ14^', d, 'html', 'tag', 'b', '<b>'));
              }
            } else {
              if (within.two) {
                exit.two();
                send(XXX_new_token('^æ15^', d, 'html', 'tag', 'b', '</b>'));
                enter.one(d.start);
                send(XXX_new_token('^æ16^', d, 'html', 'tag', 'i', '<i>'));
              } else {
                enter.two(d.start);
                send(XXX_new_token('^æ17^', d, 'html', 'tag', 'b', '<b>'));
                enter.one(d.start + 2);
                send(XXX_new_token('^æ18^', {
                  start: d.start + 2,
                  stop: d.stop
                }, 'html', 'tag', 'i', '<i>'));
              }
